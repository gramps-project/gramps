...
from ..widgets import MonitoredDate, PlaceTypeSelector
...
#
# -------------------------------------------------------------------------
class EditPlaceType(EditSecondary):
    """
    ...
    def _local_init(self):
        self.top = Glade()
        self.set_window(
            self.top.toplevel, self.top.get_object("title"), _("Place Type Editor")
        )
        self.setup_configs("interface.place-type", 450, 450)

    def _setup_fields(self):
        self.place_type = PlaceTypeSelector(
            self.dbstate, self.top.get_object("place_type"), self.obj
        )
