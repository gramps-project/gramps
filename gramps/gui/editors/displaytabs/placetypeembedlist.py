...
class PlaceTypeEmbedList(EmbeddedList):
    ...
    def __init__(self, dbstate, uistate, track, data, config_key, update):
        self.data = data
        self.update = update
    ...
    def add_button_clicked(self, obj):
        ...
        try:
            EditPlaceType(
                self.dbstate, self.uistate, self.track, ptype, self.add_callback
            )
    ...
    def edit_button_clicked(self, obj):
        ...
        if ptype:
            try:
                EditPlaceType(
                    self.dbstate, self.uistate, self.track, ptype, self.edit_callback
                )
            except WindowActiveError:
                return

    def edit_callback(self, _ptype):
        """
