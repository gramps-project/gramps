...
def parse_place_hierarchy(db: "DbGeneric", text: str) -> list[dict]:
    ...
    most-general.  Each dict contains:
    ...
    only considered a match when it is a child of whatever resolves as its
    parent. Resolution proceeds outer-to-inner so parent handles are known
    before searching for children.

    :param db: The Gramps database to search.
    :type db: DbGeneric
    :param text: Comma-separated place names, most-specific first.
    :type text: str
    :returns: List of dicts describing each token.
    :rtype: list[dict]
    """
    ...
    # Resolve from the outermost (last token) inward so parent handles are
    # known before we search for children.
    for i in range(len(entries) - 1, -1, -1):
        parent_handle: PlaceHandle | None = (
            entries[i + 1]["handle"] if i + 1 < len(entries) else None
        )
...
def commit_place_hierarchy(
    ...
    *with_transaction* must be an active :class:`DbTxn` (the caller owns the
    ...
    Returns the handle of the most-specific place (``parsed[0]``), whether it
    was pre-existing or newly created, or ``None`` if *parsed* is empty.
    ...
    # Build from outermost → innermost so parent handles exist when we need
    # to attach a PlaceRef to a child.
    for i in range(len(parsed) - 1, -1, -1):
        ...
        parent_handle: PlaceHandle | None = (
            parsed[i + 1]["handle"] if i + 1 < len(parsed) else None
        )
