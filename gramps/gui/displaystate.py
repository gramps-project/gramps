...
class History(Callback):
    """
    A linear navigation history class that manages the objects of a certain
    type that have been viewed, with ability to go back, or forward. When
    accessing an object, it should be pushed on the History.
    """
    ...
    def __init__(self, dbstate, nav_type):
        ...
        self.dbstate.connect("database-changed", self.connect_signals)
        self.signal_map = {}
    ...
    def connect_signals(self, dbstate):
        """
        Connect database signals when the database has changed.
        """
        for sig, callback in self.signal_map.items():
            dbstate.connect(sig, callback)

    def load(self):
        """
        Load the saved history if possible.
        """
        if self.dbstate.is_open():
            self.history = self.dbstate.db.get_navigation_state(
                "history", self.nav_type
            )
            self.mru = self.dbstate.db.get_navigation_state(
                "mru", self.nav_type
            )

    def clear(self):
        """
        Clear the history by resetting it.
        """
        self.history = []
        self.mru = []
        self.index = -1
        ...
        if (
            not self.history
            and self.dbstate.is_open()
            and self.nav_type == "Person"
        ):
            initial_person = self.dbstate.db.find_initial_person()
            if initial_person:
                self.push(initial_person.get_handle())

    def save(self, history=True, mru=True):
        """
        Save the history.
        """
        if history:
            self.dbstate.db.set_navigation_state(
                "history", self.nav_type, self.history
            )
        if mru:
            self.dbstate.db.set_navigation_state(
                "mru", self.nav_type, self.mru
            )

    def push(self, handle):
        """
        Push the handle on the history stack.
        """
        self.prune()
    ...
    def at_end(self):
        """
        Return true if at end of the history list.
        """
...
class RecentDocsMenu:
    """
    Menu for displaying and selecting a recently opened database.
    """
    ...
    def load(self, item):
        ...
        """
        try:
            self.fileopen(item.get_path())
        except Exception as err:
            ErrorDialog(
                _("Cannot load database"), str(err), parent=self.uistate.window
            )
    ...
    def build(self, update_menu=True):
        ...
        for item in rfiles:
            try:
                ...
                menubar += _RCT_BAR % (action_id, title)
            except RuntimeError:
                # ignore no longer existing files
                _LOG.info(
                    "Ignoring the RecentItem %s (%s)" % (title, filename)
                )

            count += 1
        menu += _RCT_BTM
        menubar += _RCT_BAR_BTM
        self.ui_xml = [menu, menubar]
...
class DisplayState(Callback):
    """
    Class to manage the overall display state.
    """
    ...
    def __init__(self, window, status, uimanager, viewmanager=None):
        ...
        self.rhandler = WarnHandler(
            capacity=400, button=warnbtn, parent=window
        )
    ...
    def set_relationship_class(self):
        """
        Rebind relationship to the current relationship calculator.
    ...
    def __develop_warn(self, _dummy_button, warning_type):
        """
