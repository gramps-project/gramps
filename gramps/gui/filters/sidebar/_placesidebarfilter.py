...
class PlaceSidebarFilter(SidebarFilter):
    def __init__(self, dbstate, uistate, clicked):
        ...
        self.place_type = PlaceTypeSelector(
            self.dbstate, self.ptype, self.filter_place.get_type(), sidebar=True
        )
        if dbstate.is_open():
            self.dbstate.db.connect(
                "custom-type-changed", self.place_type.update_models
            )
    ...
    def on_db_changed(self, _db):
        self.dbstate.db.connect("custom-type-changed", self.place_type.fill_models)
        self.place_type.fill_models()
    ...
    def get_filter(self):
        ...
        ptype = self.filter_place.get_type()
        ptype = ptype.name if ptype.is_custom() else ptype.pt_id
        ...
        empty = not (
            gid or name or ptype or enclosed or regex or within[0] or tag or gen
        )
        if empty:
            generic_filter = None
        else:
            ...
            rule = HasData([name, ptype], use_regex=regex, use_case=usecase)
            generic_filter.add_rule(rule)
