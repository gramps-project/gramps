...
def wrapper(method):
    ...
    def wrapped(*args,**keywargs):
        ...
        LOG.debug('calling {}.{}()... from file {}, line {} in {}'
...
class DummyDb(M_A_M_B("NewBaseClass", (DbReadBase, Callback, object,), {})):
    ...
    def close(self, update=True, user=None):
        ...
        if not self.db_is_open:
            LOG.warn("database is already closed")
        else:
            self.db_is_open = False
    ...
    def get_event_from_gramps_id(self, val):
        ...
        if not self.db_is_open:
            LOG.warn("database is closed")
        LOG.warn("gramps_id %s does not exist in the dummy database" % val)
        return None
    ...
    def get_tag_from_name(self, val):
        ...
        if not self.db_is_open:
            LOG.warn("database is closed")
        LOG.warn("tag name %s does not exist in the dummy database" % val)
        return None
