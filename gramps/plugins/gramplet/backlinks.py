...
class Backlinks(Gramplet):
    ...
    def build_gui(self):
        ...
        titles = [(_('Type'), 1, 100),
                  (_('Name'), 2, 100),
                  (_('Date'), 3, 200),
                  ('', 4, 1), #hidden column for the handle
                  ('', 5, 1)] #hidden column for non-localized object type
        self.model = ListModel(top, titles, event_func=self.cb_double_click)
        self.date_column = top.get_column(2)
        top.get_column(1).set_expand(True) # The name use the max possible size
        return top
    ...
    def display_backlinks(self, active_handle):
        ...
        for classname, handle in \
            ...
            if classname == "Event":
                ...
                date = displayer.display(obj.get_date_object())
                self.evts = True
            else:
                date = ""
            self.model.add((_(classname), name, date, handle, classname))
        if self.evts:
    ...
    def cb_double_click(self, treeview):
        ...
        (objclass, handle) = (model.get_value(iter_, 4),
                              model.get_value(iter_, 3))
