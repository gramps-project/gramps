...
import time
import copy
from typing import Any, cast, Generator, Iterator, Protocol
...
class DBAPI(DbGeneric):
    ...
    def _get_metadata(self, key: str, default: Any = "_") -> Any:
        """
    ...
    def _set_metadata(self, key: str, value: Any, use_txn: bool = True) -> None:
        """
        key: string
    ...
    def _commit_raw(self, data: AnyDataDict, obj_key: int) -> None:
        """
    ...
    def _update_backlinks(self, obj: AnyObject, transaction: DbTxn) -> None:
        if not transaction.batch:
    ...
    def _remove_backlinks(
        self, obj_class: str, obj_handle: AnyHandle, transaction: DbTxn
    ) -> None:
        """
    ...
    def _has_handle(self, obj_key: int, handle: AnyHandle) -> bool:
        table = KEY_TO_NAME_MAP[obj_key]
        self.dbapi.execute(f"SELECT 1 FROM {table} WHERE handle = ?", [handle])
        return self.dbapi.fetchone() is not None

    def _has_gramps_id(self, obj_key: int, gramps_id: PrimaryObjectGrampsID) -> bool:
        table = KEY_TO_NAME_MAP[obj_key]
    ...
    def _get_raw_data(self, obj_key: int, handle: AnyHandle):
        table = KEY_TO_NAME_MAP[obj_key]
        self.dbapi.execute(
    ...
    def _get_raw_from_id_data(self, obj_key: int, gramps_id: PrimaryObjectGrampsID):
        table = KEY_TO_NAME_MAP[obj_key]
        self.dbapi.execute(
    ...
    def _update_secondary_values(self, obj: AnyObject) -> None:
        """
