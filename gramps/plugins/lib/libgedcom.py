...
class GedcomParser(UpdateCallback):
        ...
        if sub_state.place:
            ...
            if len(places) == len(ptypes):
                ...
                for indx in reversed(range(len(places))):
                    ...
                    place = self.__find_place(title, ptypes[indx], prev_place_hndl)
                ...
                place = self.__find_place(
                    sub_state.place.title, sub_state.place.place_type, None
                )
                if place is None:
                    place = sub_state.place
                    self.dbase.add_place(place, self.trans)
        ...
        def __check(_map, has_gid_func, class_func, commit_func, gramps_id2handle, msg):
            for input_id, gramps_id in _map.map().items():
                ...
                if not has_gid_func(gramps_id):
                    _handle = self.__find_hndl_from_id(gramps_id, gramps_id2handle)
                    if msg == "FAM":
                        make_unknown(
                            gramps_id,
    ...
    def __event_addr(self, line, state):
        ...
        if not title:  # indicates that no structured address provided
            ...
            # structured address provided
            for item, ptype in (
                (addr.get_street(), PlaceType.STREET),
