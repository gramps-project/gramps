...
_descendants = ["", "otrok", "vnuk(inja)", "pravnuk(inja)", "prapravnuk(inja)"]
...
_siblings = ["", "sorojenci", "strici", "stari strici", "prastrici", "praprastrici"]
...
class RelationshipCalculator(gramps.gen.relationship.RelationshipCalculator):
    ...
    def getNiece(self, distance, level):
        if distance == 5 and level == 1:
            ...
            return "%s-krat-pra-nečakinja%s" % (
                level - 1,
                self.getSuffix(distance, level),
            )
        else:
    ...
    def get_single_relationship_string(
        ...
        if Gb == 0:
            ...
            if gender_b == Person.FEMALE:
                ...
                rel_str = self.getUncle(
                    Ga + Gb, Ga - Gb
                )  # we'll use male for unknown sex
        else:  # Ga < Gb
            if gender_b == Person.FEMALE:
                rel_str = self.getNiece(Ga + Gb, Gb - Ga)
            else:
                rel_str = self.getNephew(
                    Ga + Gb, Gb - Ga
                )  # we'll use male for unknown sex
        return rel_str
