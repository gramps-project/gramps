...
class TreeOptions:
    ...
    def add_menu_options(self, menu):
        ...
        preferred_name.set_help(
            _(
                "Select the portion of the given name to be "
        ...
        nodesize.set_help(
            _(
                "One dimension of a node, in mm. If the timeflow "
...
class TreeDocBase(BaseDoc, TreeDoc):
    ...
    def write_node(self, db, level, node_type, person, marriage_flag, option_list=None):
        ...
        if self.name_format == "1":
            ...
            self.write(
                level + 1,
                "name = {{{}}},\n".format(" ".join([e for e in name_parts if e])),
            )
            ...
            self.write(
                level + 1,
                "name = {{{}}},\n".format(", ".join([e for e in name_parts2 if e])),
            )
