...
_COMPOUND_QUAL_MOD_TW: dict[str, tuple[int, int]] = {
    "估計早於": (Date.QUAL_ESTIMATED, Date.MOD_BEFORE),
    "估計晚於": (Date.QUAL_ESTIMATED, Date.MOD_AFTER),
    "推算早於": (Date.QUAL_CALCULATED, Date.MOD_BEFORE),
    "推算晚於": (Date.QUAL_CALCULATED, Date.MOD_AFTER),
}
...
class DateParserZH_TW(DateParser):
    ...
    quality_to_int = {
        ...
        "估計為": Date.QUAL_ESTIMATED,
        "推算為": Date.QUAL_CALCULATED,
    }
    ...
    def __init__(self, *args: object, **kwargs: object) -> None:
        ...
        quality+modifier prefix (e.g. "估計早於") is recognised, then
    ...
    def init_strings(self) -> None:
        ...
        # The dhformat "西元%Y年%m月%d日" produces display strings like
        # "西元2000年3月". The non-capturing (?:西元)? prefix allows the
        # numeric parser to round-trip those strings correctly.
        self._numeric = re.compile(r"(?:西元)?((\d+)年\s*)?((\d+)月\s*)?(\d+)?日?\s*$")
        ...
        # display strings like "估計為1800年" parse without a separating space.
        ...
        # display strings like "大約1850年" parse without a separating space.
    ...
    def match_quality(self, text: str, qual: int) -> tuple[str, int]:
        ...
        Compound strings like "估計早於" encode both QUAL_ESTIMATED and
        MOD_BEFORE.  When one is found, the modifier is stashed in
        _pending_modifier for match_modifier to consume.
        """
        for compound, (q, m) in _COMPOUND_QUAL_MOD_TW.items():
...
class DateDisplayZH_TW(DateDisplay):
    ...
    def __init__(self, *args: object, **kwargs: object) -> None:
        """
        Initialize and restore Traditional Chinese lunar month names.
    ...
    def display_formatted(self, date: Date) -> str:
        """
        Return a text string representing the date in Traditional Chinese.
        ...
        MOD_BEFORE) produce a single fused prefix like "估計早於".
        """
