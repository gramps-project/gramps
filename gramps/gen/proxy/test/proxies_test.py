...
class PrivateProxyTest(unittest.TestCase):
    ...
    def remove_object_item(self, data):
        if isinstance(data, dict):
            new_data = {}
            for key, value in data.items():
                if key == "_object":
                    continue
                new_data[key] = self.remove_object_item(value)
            return new_data
        elif isinstance(data, list):
            new_list = []
            for item in data:
                new_list.append(self.remove_object_item(item))
            return new_list
        else:
            return data
    ...
    def assertDataEquals(self, data1, data2):
        ...
        self.assertEquals(
            self.remove_object_item(data1),
            self.remove_object_item(data2),
        )
