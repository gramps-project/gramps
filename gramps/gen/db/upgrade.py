...
from gramps.gen.lib import EventType, NameOriginType, Tag, MarkerType, Date
...
def gramps_upgrade_20(self):
    ...
    # Modify Person
    # ----------------------------------------
    # Migrate attribute_list and attribute_list inside event_ref_list
    # and media_list
    for person_handle in self.get_person_handles():
        ...
        need_save = False
        if attribute_list:
            attribute_list = upgrade_attribute_list_20(attribute_list)
            need_save = True
        event_ref_list, changed = upgrade_event_ref_list_20(event_ref_list)
        need_save = need_save or changed
        media_list, changed = upgrade_media_list_20(media_list)
        if need_save or changed:
            new_person = (
                handle,
                gramps_id,
                gender,
    ...
    # Modify Event
    # ----------------------------------------
    # Migrate attribute_list and attribute_list inside media_list
    ...
    # Modify Place
    # ----------------------------------------
    # Migrate attribute_list inside media_list
    ...
    # ----------------------------------------
    # Migrate attribute_list inside media_list
    for citation_handle in self.get_citation_handles():
        citation = self.get_raw_citation_data(citation_handle)
        (
            handle,
            gramps_id,
            date,
...
def upgrade_event_ref_list_20(event_ref_list):
    """
    Update attribute_list in eventref objects
    """
    changed = False
    new_event_ref_list = []
    ...
    return new_event_ref_list, changed


def upgrade_media_list_20(media_ref_list):
    """
    Update attribute_list in mediaref objects
    """
    changed = False
    new_media_ref_list = []
    for media_ref in media_ref_list:
        ...
        new_media_ref_list.append((new_media_ref))
    return new_media_ref_list, changed


def upgrade_attribute_list_20(attribute_list):
    """
    Insert date into attribute objects
    """
    date = Date().serialize()
    new_attribute_list = []
    for attribute in attribute_list:
        (privacy, citation_list, note_list, the_type, value) = attribute
        new_attribute = (
            privacy,
            citation_list,
            note_list,
...
def gramps_upgrade_14(self):
    ...
    for handle in self.get_person_handles():
        ...
        new_alternate_names = [
            convert_name_14(name) for name in alternate_names
        ]
...
def new_source_list_14(source_list):
    ...
    for source in source_list:
        ...
        new_source_list.append(
            (new_date, private, note_list, confidence, ref, page)
        )
...
def new_person_ref_list_14(person_ref_list):
    ...
    for person_ref in person_ref_list:
        ...
        new_source_list = new_source_list_14(source_list)
        new_person_ref_list.append(
            (private, new_source_list, note_list, ref, rel)
        )
    return new_person_ref_list
